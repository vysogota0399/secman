...
func TestBackend_createHandler(t *testing.T) {
	...
	tests := []struct {
		...
		wantErr bool
		setup   func(*Backend, *secman.LogicalRequest)
	}{
		{
			...
			wantErr: false,
			setup: func(b *Backend, req *secman.LogicalRequest) {
				...
				b.repo.storage.(*secman.MockILogicalStorage).EXPECT().
					Update(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
					Return(nil)

				// Mock metadata Update
				b.metadata.storage.(*secman.MockBarrierStorage).EXPECT().
					Update(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
					Return(nil)
			},
		},
