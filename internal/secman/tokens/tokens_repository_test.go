...
func TestNewLogicalStorage(t *testing.T) {
	...
	tests := []struct {
		name string
		args args
		want secman.ILogicalStorage
	}{
		{
			name: "successful creation",
			args: args{
				storage: secman.NewMockBarrierStorage(ctrl),
			},
			want: &secman.LogicalStorage{},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestNewTokensRepository(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "successful creation",
			args: args{
				barrier: secman.NewMockBarrierStorage(ctrl),
			},
			want: &TokensRepository{},
		},
	}
...
func TestTokensRepository_Find(t *testing.T) {
	...
	tests := []struct {
		...
		want    Token
		wantErr bool
		prepare func(storage *secman.MockILogicalStorage)
	}{
		{
			name: "successful find",
			fields: fields{
				storage: secman.NewMockILogicalStorage(ctrl),
			},
			args: args{
	...
	tests := []struct {
	...
	}{
		{
			...
			prepare: func(storage *secman.MockILogicalStorage) {
				...
				tokenBytes, _ := json.Marshal(token)
				storage.EXPECT().
					Update(gomock.Any(), "test-key", secman.Entry{
						Value: string(tokenBytes),
						Key:   "test-key",
					}, gomock.Any()).
					Return(nil)
