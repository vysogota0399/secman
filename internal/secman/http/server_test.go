...
func TestRouter(t *testing.T) {
	...
	router := NewTestRouter(t, core, nil)
	server, app := NewTestServer(context.Background(), t, core, router)
	if err := app.Start(context.Background()); err != nil {
		t.Fatalf("Failed to start server: %v", err)
	}

	srv, ok := server.srv.(*TestHTTPServer)
	if !ok {
		t.Fatalf("Failed to cast server to TestHTTPServer")
	}

	t.Run("route without mw", func(t *testing.T) {
		core.Barrier.(*secman.MockIBarrier).EXPECT().Info().Return("sealed status")
		client := srv.Client()
	...
	coreRepository := secman.NewMockICoreRepository(secman.NewController(t))
	...
	t.Run("abort if not authorized by token", func(t *testing.T) {
		core.RootTokens.(*secman.MockIRootTokens).EXPECT().Compare(gomock.Any(), gomock.Any(), gomock.Any()).Return(assert.AnError)
		client := srv.Client()
		req, err := http.NewRequest(http.MethodPost, srv.URL+"/api/sys/unseal", nil)
		if err != nil {
			t.Fatalf("Failed to create request: %v", err)
	...
	t.Run("abort if not authorized by engine", func(t *testing.T) {
		core.Auth.(*secman.MockIAuth).EXPECT().Authorize(gomock.Any()).Return(assert.AnError)
		client := srv.Client()
		req, err := http.NewRequest(http.MethodPost, srv.URL+"/api/sys/unseal", nil)
		if err != nil {
			t.Fatalf("Failed to create request: %v", err)
