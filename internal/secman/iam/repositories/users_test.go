...
func TestNewUsers(t *testing.T) {
	...
	mockBarrier := secman.NewMockIBarrier(ctrl)
...
func TestUsers_Get(t *testing.T) {
	...
	tests := []struct {
		...
		want    User
		wantErr bool
		prepare func(storage *secman.MockIBarrier)
	}{
		{
	...
	}{
		{
			...
			prepare: func(storage *secman.MockIBarrier) {
				storage.EXPECT().
					...
					DoAndReturn(func(_ context.Context, key string, entry secman.Entry, _ time.Duration) error {
						var user User
