...
// InitCommand представляет собой команду для инициализации сервера.
// Сервер должен быть не инициализирован, иначе будет выведено сообщение о том, что сервер уже инициализирован.
// Если сервер был инициализирован, то токен будет сохранен в сессию, если указан флаг -pt, иначе его необходимо сохранить в переменную окружения ROOT_TOKEN.
// Доступные команды:
