...
// PartsBuffer представляет собой буфер для хранения частей ключа ансила SSS.
// Для выполнения ансила сервера необходимо собрать все части ключа.
...
// Aes256Bариер имплементирует интерфейс IBarrier.
// Он использует AES256 для шифрования и дешифрования данных.
...
var _ server.IBarrier = &Aes256Barier{}

func NewAes256Barier(storage server.IStorage, log *logging.ZapLogger, keyring *server.Keyring) (*Aes256Barier, error) {
	sealed := &atomic.Bool{}
	sealed.Store(true)
...
func (b *Aes256Barier) Unseal(ctx context.Context, key []byte) (bool, error) {
	...
	b.keyring.SetRootKey(&server.Key{
		Raw:    rootKey,
		Status: server.KeyStatusActive,
	})
...
func (b *Aes256Barier) Update(ctx context.Context, path string, entry server.Entry, ttl time.Duration) error {
	if b.isSealed() {
		return errors.New("barrier is sealed")
	}
	...
	return b.storage.Update(ctx, path, server.PhysicalEntry{Value: ciphertext}, ttl)
}

func (b *Aes256Barier) Get(ctx context.Context, path string) (server.Entry, error) {
	if b.isSealed() {
		return server.Entry{}, errors.New("barrier is sealed")
	}

	res, err := b.storage.Get(ctx, path)
	if err != nil {
		return server.Entry{}, fmt.Errorf("aes256 barrier: get key %s failed error: %w", path, err)
	}
	...
	if err != nil {
		return server.Entry{}, fmt.Errorf("aes256 barrier: decrypt key %s failed error: %w", path, err)
	}

	return server.Entry{Value: string(plaintext), Path: path}, nil
}

func (b *Aes256Barier) GetOk(ctx context.Context, path string) (server.Entry, bool, error) {
...
func (b *Aes256Barier) List(ctx context.Context, path string) ([]server.Entry, error) {
	if b.isSealed() {
	...
	for i, pe := range physicalEntries {
		...
		entries[i] = server.Entry{Value: string(plaintext), Path: pe.Path}
	}
...
// Init создает рутовый ключ верхнего уровня для шифрования ключей, которые исспользуются непосредственно для шифрования данных.
// Для достижения большей сохранности рутовый ключ разбивается на части. Для сборки рутового ключа необходимо собрать thresholdsCoundRequired его частей.
...
func (b *Aes256Barier) encript(message []byte, key *server.Key) ([]byte, error) {
...
func (b *Aes256Barier) decript(message []byte, key *server.Key) ([]byte, error) {
	block, err := aes.NewCipher(key.Raw)
	if err != nil {
		return nil, err
	}
...
func (b *Aes256Barier) actualKey() *server.Key {
	return b.keyring.GetKey(b.keyring.ActualID())
}

func (b *Aes256Barier) keyFromCiphertext(ciphertext []byte) (*server.Key, error) {
...
func (b *Aes256Barier) persistKey(ctx context.Context, key []byte, id uint32) error {
	return b.storage.Update(ctx, path.Join(server.KeyringPath, strconv.Itoa(int(id))), server.PhysicalEntry{Value: key}, 0)
}

func (b *Aes256Barier) initKeys() error {
	keys, err := b.storage.List(context.Background(), server.KeyringPath)
	if err != nil {
