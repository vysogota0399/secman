...
// mockStorage implements server.IStorage for testing
type mockStorage struct{}

func (m *mockStorage) Get(ctx context.Context, path string) (server.PhysicalEntry, error) {
	return server.PhysicalEntry{}, nil
}

func (m *mockStorage) Update(ctx context.Context, path string, value server.PhysicalEntry, ttl time.Duration) error {
	return nil
}
...
func (m *mockStorage) List(ctx context.Context, path string) ([]server.PhysicalEntry, error) {
	return nil, nil
}
...
func NewBarier(t *testing.T, keyring *server.Keyring) (*Aes256Barier, *server.MockIStorage) {
	t.Helper()
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()

	storage := server.NewMockIStorage(ctrl)
	barrier, err := NewAes256Barier(storage, server.NewLogger(t), keyring)
	assert.NoError(t, err)
...
func TestAes256Barier_Unseal(t *testing.T) {
	...
	tests := []struct {
		...
		want    bool
		wantErr bool
		prepare func(b *Aes256Barier, a *args, storage *server.MockIStorage)
	}{
		{
		...
		},
		{
			...
			prepare: func(b *Aes256Barier, a *args, storage *server.MockIStorage) {
				...
				var tmpValue server.PhysicalEntry
				storage.EXPECT().Update(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(func(ctx context.Context, path string, value server.PhysicalEntry, ttl time.Duration) error {
					tmpValue = value
					return nil
				})

				storage.EXPECT().List(gomock.Any(), gomock.Any()).Return([]server.PhysicalEntry{tmpValue}, nil).DoAndReturn(func(ctx context.Context, path string) ([]server.PhysicalEntry, error) {
					return []server.PhysicalEntry{tmpValue}, nil
				})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			storage := server.NewMockIStorage(ctrl)
			b, err := NewAes256Barier(storage, server.NewLogger(t), server.NewKeyring())
			if err != nil {
...
func TestAes256Barier_List(t *testing.T) {
	...
	tests := []struct {
		...
		want    []server.Entry
		wantErr bool
		prepare func(b *Aes256Barier, storage *server.MockIStorage)
	}{
		{
			name: "barrier is sealed",
			fields: fields{
