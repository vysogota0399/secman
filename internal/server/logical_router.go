...
// ILogicalRouter интерфейс для получения движков по заданным маршрутам.
type ILogicalRouter interface {
	// Register регистрирует движок в роутере.
	Register(ctx context.Context, engine LogicalBackend) error
	// Resolve получает движок по заданному маршруту.
	Resolve(path string) (LogicalBackend, error)
	// PostUnsealEngines инициализирует движки после разблокировки барьера.
	PostUnsealEngines(ctx context.Context) error
	// EnableEngine включает движок и инициализирует его роутер.
