...
var (
	...
	_ server.AuthorizeBackend = &Backend{}
)
...
func (b *Backend) Paths() map[string]map[string]*server.Path {
	paths := make(map[string]map[string]*server.Path)

	// Login path
	loginPath := PATH + "/login"
	if _, ok := paths[http.MethodPost]; !ok {
		paths[http.MethodPost] = make(map[string]*server.Path)
	}
	paths[http.MethodPost][loginPath] = &server.Path{
	...
	paths[http.MethodPost][registerPath] = &server.Path{
	...
	if _, ok := paths[http.MethodGet]; !ok {
		paths[http.MethodGet] = make(map[string]*server.Path)
	}
	paths[http.MethodGet][paramsPath] = &server.Path{
	...
	if _, ok := paths[http.MethodPut]; !ok {
		paths[http.MethodPut] = make(map[string]*server.Path)
	}
	paths[http.MethodPut][paramsPath] = &server.Path{
		Handler:     nil,
...
func (b *Backend) PostUnseal(ctx context.Context) error {
	...
	if !ok {
		return fmt.Errorf("logopass: post unseal failed error: %w", server.ErrEngineIsNotEnabled)
	}
