...
var _ server.IStorage = &RedisStorage{}
...
func (s *RedisStorage) Get(ctx context.Context, path string) (server.PhysicalEntry, error) {
	...
	if err != nil {
		return server.PhysicalEntry{}, fmt.Errorf("storage: failed to get entry with key %s: %w", path, err)
	}

	return server.PhysicalEntry{Value: res, Path: path}, nil
}

func (s *RedisStorage) Update(ctx context.Context, path string, entry server.PhysicalEntry, ttl time.Duration) error {
...
func (s *RedisStorage) List(ctx context.Context, path string) ([]server.PhysicalEntry, error) {
	var entries []server.PhysicalEntry
	var cursor uint64
	var err error
	...
	for {
		...
		if len(paths) > 0 {
			...
			for i, cmd := range cmds {
				val, err := cmd.Bytes()
				if err == nil {
					entries = append(entries, server.PhysicalEntry{Value: val, Path: paths[i]})
				}
			}
		}
