...
func TestUnseal_Handler(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			prepare: func(core *server.Core) {
				core.IsSealed.Store(true)
				core.Barrier.(*server.MockIBarrier).EXPECT().Unseal(gomock.Any(), gomock.Any()).Return(true, assert.AnError)
			},
		...
		},
		{
			...
			prepare: func(core *server.Core) {
				core.IsSealed.Store(true)
				core.Barrier.(*server.MockIBarrier).EXPECT().Unseal(gomock.Any(), gomock.Any()).Return(true, nil)
				core.Router.(*server.MockILogicalRouter).EXPECT().PostUnsealEngines(gomock.Any()).Return(nil)
				core.Auth.(*server.MockIAuth).EXPECT().PostUnseal(gomock.Any(), core.Router).Return(nil)
			},
			body:       strings.NewReader("{\"key\": \"3bPuOMYNtgI9osmR60PrKBvz27fhNYc6E+pZJVFRhgs=\"}"),
