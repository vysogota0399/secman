...
type EnableEngine struct {
	core   *server.Core
	router server.ILogicalRouter
	log    *logging.ZapLogger
}

func NewEnableEngine(core *server.Core) *EnableEngine {
...
func (h *EnableEngine) Handler() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		resp, err := h.router.EnableEngine(c.Request.Context(), be, &server.LogicalRequest{Context: c})
		if err != nil {
			if errors.Is(err, server.ErrEngineNotFound) {
				c.JSON(http.StatusNotFound, gin.H{"error": "engine " + enginePath + " not found"})
				return
			}
