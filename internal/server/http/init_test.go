...
func TestInit_Handler(t *testing.T) {
	tests := []struct {
		name       string
		prepare    func(core *server.Core, coreRepository *server.MockICoreRepository)
		wantStatus int
	}{
		{
		...
		},
		{
			...
			prepare: func(core *server.Core, coreRepository *server.MockICoreRepository) {
				...
				core.RootTokens.(*server.MockIRootTokens).EXPECT().Gen(gomock.Any(), gomock.Any()).Return("", assert.AnError)
			},
		...
		},
		{
			...
			prepare: func(core *server.Core, coreRepository *server.MockICoreRepository) {
				core.IsInitialized.Store(false)
				core.RootTokens.(*server.MockIRootTokens).EXPECT().Gen(gomock.Any(), gomock.Any()).Return("root-token", nil)
				core.Barrier.(*server.MockIBarrier).EXPECT().Init(gomock.Any()).Return(nil, assert.AnError)
			},
			wantStatus: http.StatusInternalServerError,
		},
		{
			...
			prepare: func(core *server.Core, coreRepository *server.MockICoreRepository) {
				core.IsInitialized.Store(false)
				core.RootTokens.(*server.MockIRootTokens).EXPECT().Gen(gomock.Any(), gomock.Any()).Return("root-token", nil)
				core.Barrier.(*server.MockIBarrier).EXPECT().Init(gomock.Any()).Return([][]byte{[]byte("unseal-token")}, nil)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			coreRepository := server.NewMockICoreRepository(gomock.NewController(t))
