...
func TestBackend_ShowExpiryDateHandler(t *testing.T) {
	...
	}{
		{
			...
			prepare: func(mockStorage *secman.MockILogicalStorage, b *Backend) {
				mockStorage.EXPECT().
					...
					Return(secman.Entry{Value: "12/25"}, true, nil)
			},
		},
		{
			name: "expiry date not found",
			fields: &fields{
				exist:    &atomic.Bool{},
				router:   nil,
				metadata: &MetadataRepository{},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := b.ShowExpiryDateHandler(tt.args.ctx, tt.args.req, tt.args.params)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
